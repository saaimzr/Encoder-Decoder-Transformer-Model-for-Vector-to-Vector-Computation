...
def generate_token_dict(vocab):
    ...
    ##############################################################################
    token_dict = {vocab[k]: k for k in range(len(vocab))}
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return token_dict
...
def scaled_dot_product_two_loop_single(
    ...
    ###############################################################################
    # initialise a scaled_weights matrix that we'll populate
    K, M = query.shape
    scaled_weights = torch.zeros((K,K), dtype=query.dtype, device=query.device)
    
    # Scale weights by 1/sqrt(M)
    scale = 1/torch.sqrt(torch.Tensor([M]))

    # Get dot products between query and key
    for q in range(K): # Query index
        for k in range(K): # Key index
            scaled_weights[q, k] = query[q].dot(key[k])*scale
    
    # Apply softmax to all
    sm = torch.nn.Softmax(dim=1)
    scaled_weights = sm(scaled_weights)

    # Multiply scaled weights by values
    out = scaled_weights.mm(value)

    """ 
        No loops is easier and equivalent! 
        The code below uses no loops for the exact same operation as above.
    """
    # # We need `M` for scaling the weights
    # _, M = query.shape

    # # Matrix multiply of query and key(transpose)
    # scaled_weights = query.mm(key.t())/torch.sqrt(torch.Tensor([M]))

    # # Apply softmax to weights along k dimension
    # s = torch.nn.Softmax(dim=1)
    # scaled_weights = s(scaled_weights)

    # # Multiply scaled weights by value
    # out = scaled_weights.mm(value)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
def scaled_dot_product_two_loop_batch(
    ...
    ###############################################################################
    # Initalise scaled weights matrix.
    scaled_weights = torch.zeros((N, K, K), dtype=query.dtype, device=query.device)

    # Initialise scaling coefficient; 1/sqrt(M).
    scale = 1/torch.sqrt(torch.Tensor([M]))

    # Populate scaled weights matrix
    for q in range(K):
        for k in range(K):
            # Hadamard product between `query` and `key` matrices of shape (N, M),
            # followed by a sum along M to get a vector, of dot products, of shape 
            # (N,).
            scaled_weights[:, q, k] = torch.sum(query[:, q]*key[:, k], dim=1)*scale

    # Run scaled weights through softmax, along key dimension.
    sm = torch.nn.Softmax(dim=2)
    scaled_weights = sm(scaled_weights)

    # Batch matrix multiply scaled weights by value
    out = scaled_weights.bmm(value)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Initialis scaling variable; 1/sqrt(M).
    scale = 1/torch.sqrt(torch.Tensor([M]))

    # Get the scaled weights; under name `weights_softmax` now
    weights_softmax = query.bmm(key.transpose(1,2))*scale
    ...
    if mask is not None:
        ...
        ##########################################################################
        weights_softmax[mask] = -1e9
    
    # Copy-paste from 2-loop version
    # Run scaled weights through softmax, along key dimension.
    sm = torch.nn.Softmax(dim=2)
    weights_softmax = sm(weights_softmax)
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        ##########################################################################
        # Initialise transformation MLP layers.
        # Using psk (my own little library) due to different weight initialisations.
        self.q = psk.Linear(dim_in, dim_q)
        self.k = psk.Linear(dim_in, dim_q)
        self.v = psk.Linear(dim_in, dim_v)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        self.decoder = psk.Linear(num_heads*dim_out, dim_in)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        for head in self.heads:
            l.append(head(query, key, value, mask))
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        ##########################################################################
        # Get the mean and variance of shape (N, K)
        # If the shape is already (N, K), what do you do? return x? no scale/shift?
        if x.dim() == 2:
            N, K = x.shape

            # Get the mean
            mu = x.sum(dim=1).unsqueeze(dim=1)/K

            # Get the SD.
            std = torch.sqrt(torch.sum((x-mu)**2, dim=1)/K).unsqueeze(dim=1)

            # Normalise
            y = (x - mu)/(std + self.epsilon)

            # Scale and Shift
            y = y*self.scale + self.shift

        else:
            # POTENTIAL ERROR: need to take mean and SD over K aswell
            N, K, M = x.shape

            # Get the mean.
            # Should be shape (N, K, 1), good for broadcasting
            mu = x.sum(dim=2).unsqueeze(dim=2)/(M)

            # Get the SD.
            std = torch.sqrt(torch.sum((x - mu)**2, dim=2)/(M)).unsqueeze(dim=2)
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        self.MLP = nn.Sequential(
            psk.Linear(inp_dim, hidden_dim_feedforward),
            nn.ReLU(),
            psk.Linear(hidden_dim_feedforward, inp_dim)
        )
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        # 2. Layer Normalisation layers; could make them into a ModuleList, but
        #    there's no point for the time being.
        self.LN1 = LayerNormalization(emb_dim)
        self.LN2 = LayerNormalization(emb_dim)

        # 3. Feedforward block.
        self.MLP = FeedForwardBlock(emb_dim, feedforward_dim)
    ...
    def forward(self, x):
        ...
        # 3. Dropout - save output for residual connection
        res = self.dropout(y)
...
def get_subsequent_mask(seq):
    ...
    # Initialise the mask
    mask = torch.triu(torch.ones((N, K, K), device=seq.device, dtype=torch.bool), diagonal=1)
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        self.norm3 = None
        self.dropout = None
        self.feed_forward = None # PYSKINAS: this seems redundant
        ##########################################################################
        # TODO: Initialize the following layers:                                 #
    ...
    def forward(
        ...
        # 3. Dropout - makes a residual element we need to keep track of
        res = self.dropout(y)

        # 5. MultiHead Attention
        y = self.attention_cross(res, enc_inp, enc_inp)
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    y = (torch.arange(K)/K).tile((M, 1)).t().unsqueeze(dim=0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    a = torch.arange(M)//M # These will always be all 0, why?
    den = (10000**a).unsqueeze(dim=0) # denominator
    f = p/den # fraction

    # Even steps are sine
    f[:, torch.arange(0, M, step=2)] = torch.sin(f[:, torch.arange(0, M, step=2)])

    # Odd steps are cosine
    f[:, torch.arange(1, M, step=2)] = torch.cos(f[:, torch.arange(1, M, step=2)])

    # Unsqueeze to get shape (1, K, M)
    y = f.unsqueeze(dim=0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        N, _, _ = ques_pos.shape
        O  = dec_out.shape[1]
        dec_out = dec_out.reshape(N*O, -1)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
